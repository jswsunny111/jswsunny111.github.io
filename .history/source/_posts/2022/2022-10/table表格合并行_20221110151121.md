...
### 实现element table表格合并
![E_J_tableHb_1](/img/2022/print_2.png)
 饿了么框架介绍的基础，合并的主要属性是 span-method方法 返回的 rowspan，colspan 值是做依赖合并的依据
```
 <ez-table
      :data="tableData"
...
#### 主要问题/解决方案
  ...
  2. 当需要合并时的依据 识别ID 条件是什么？
  
  判断首先合并的场景，是相邻的某个 key 值相同的情况下，进行合并。可以理解为当前这一条与上一条数据 是否相同进行合并
  （PS：首先确保数据是否排序过或者一样的不在一起需不需要合并，注意处理边界线） 
   那么我们可以 给 数据加一个 count值 去动态计算合并的行数，当满足条件给数值进行累加 
   
#### 功能实现
##### 常规表格数据
 ![E_J_tableHb_3](/img/2022/print_2.png)
```
//实际业务，以‘serialNo’做识别key值比对，数据中只需要相邻的‘adviceNo’值一致的合并。
<ez-table
        border
        size="mini"
        :data="tabdata"
...
```
##### 嵌套数据结构
#####  ![E_J_tableHb_2](/img/2022/print_2.png)
```
